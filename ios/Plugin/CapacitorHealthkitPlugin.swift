...
public class CapacitorHealthkitPlugin: CAPPlugin {
    ...
    let statisticsQuery = HKStatisticsQuery(quantityType: sampleType as! HKQuantityType,
                                            quantitySamplePredicate: predicate,
                                            options: .cumulativeSum) { (_, result, error) in
        guard let result = result else {
            return completion(.failure(error ?? HKSampleError.sampleTypeFailed))
        }

        // Отримання основних статистичних показників
        let sum = result.sumQuantity()?.doubleValue(for: HKUnit.meter()) ?? 0
        let unit = HKUnit.meter() 

        // Дата початку та закінчення періоду
        let startDate = result.startDate
        let endDate = result.endDate

        completion(.success([
            "countReturn": 1, // Кількість результатів у відповіді (1, оскільки це статистика)
            "resultData": [
                "sumDistance": sum,
                "unit": unit.unitString, // Одиниці вимірювання
                "startDate": startDate,
                "endDate": endDate,
            ],
        ]))
